...
for a = [5, 6, 7, 8, 10, 15, 20, 30, 40, 50]     % drop radii, in um
    ...
    for ev = [-2, 0, 2, 4]
        colors = spec_to_rgb([lambda(:) * 1000, intensity' .* sun_spec' * 2^ev * lambda_num], 'Y', -1);
        ...
        drawnow;
        saveas(gcf, sprintf('../img/corona_mie_r%04d_e%+2d.png', a, ev));
        ...
        imshow(img);
        drawnow;
        imwrite(uint8(img * 256), sprintf('../img/corona_r%04d_e%+2d.png', a, ev));
    end
end
